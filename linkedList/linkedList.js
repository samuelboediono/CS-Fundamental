...
LinkedList.prototype.insert = function(val, prevNode){
  ...
  if(!prevNode){ // if there was no value passed as an argument to the prevNode parameter
    if(!this.head){   //if head is null
      ...
      node.next = this.head.next; //set the new node's 'next' field to be set to the heads next which was null
    }
    this.head = node; // if the head was null then set the head node to be the new node 
